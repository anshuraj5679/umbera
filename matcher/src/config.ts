...
const envSchema = z.object({
  ...
  MATCHER_REQUIRE_BATCH_PROOF_ANCHOR_FOR_SETTLEMENT: envBool.default(false),
  MATCHER_AUDIT_REPAIR_INTERVAL_SEC: z.coerce.number().int().min(0).default(300),
  MATCHER_INVARIANT_RECONCILE_INTERVAL_SEC: z.coerce.number().int().min(0).default(60),
  MATCHER_RETRY_WORKER_INTERVAL_SEC: z.coerce.number().int().min(0).default(20),
  MATCHER_RETRY_WORKER_BATCH_SIZE: z.coerce.number().int().min(1).max(250).default(50),
  MATCHER_TASK_LEASE_SEC: z.coerce.number().int().min(10).max(900).default(120),
  MATCHER_STALE_TASK_SWEEP_INTERVAL_SEC: z.coerce.number().int().min(0).default(60),
  MATCHER_STALE_TASK_AFTER_SEC: z.coerce.number().int().min(30).max(3600).default(300),
