...
console.log('dustSpend.oldNullifier type:', typeof dustSpend.oldNullifier);
console.log('dustSpend.oldNullifier:', dustSpend.oldNullifier?.toString?.());
console.log('dustSpend.oldNullifier keys:', Object.getOwnPropertyNames(dustSpend.oldNullifier || {}));
// Try inspecting it via the ledger wasm
const nullifier = dustSpend.oldNullifier;
if (nullifier) {
  // DustNullifier should have toString
  const nullStr = nullifier.toString();
  console.log('nullifier.toString():', nullStr);
}