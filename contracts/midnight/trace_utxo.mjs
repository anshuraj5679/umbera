...
console.log('After funding: utxos =', localState.utxos.length);
...
console.log('After deploy 1: utxos =', localState.utxos.length, 'mtIndex:', localState.utxos[0]?.mtIndex);

console.log('Applying comm2...');
localState = localState.applyCommitmentCollapsedUpdate(
  ledger.DustStateMerkleTreeCollapsedUpdate.deserialize(Buffer.from(d.data.comm2.update, 'hex'))
);
console.log('After comm2: utxos =', localState.utxos.length);
...
const rep2 = localState.replayRawEvents(dustSecretKey, Buffer.from(DEPLOY2_EVENT_HEX, 'hex'));
console.log('rep2 class:', rep2.constructor.name);
console.log('rep2 proto:', Object.getOwnPropertyNames(Object.getPrototypeOf(rep2)));
const newState = rep2.state;
console.log('newState utxos:', newState.utxos.length);
for (const u of newState.utxos) {
  console.log('  UTXO mtIndex:', u.mtIndex);
}
console.log('newState balance:', newState.walletBalance(new Date()).toString());
