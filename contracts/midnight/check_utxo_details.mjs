...
console.log('UTXO 0 (funding):', Object.getOwnPropertyNames(localState.utxos[0]));
console.log('UTXO 0 backingNight:', localState.utxos[0].backingNight);
console.log('UTXO 0 ctime:', localState.utxos[0].ctime);
...
// Fetch the rest of block 884520:
const qRest = `query {
  c: dustCommitmentMerkleTreeUpdate(startIndex: 115345, endIndex: 115345) { update }
  g: dustGenerationMerkleTreeUpdate(startIndex: 13107, endIndex: 13107) { update }
}`;
const rRest = await fetch(env.MIDNIGHT_INDEXER_URL, {
  method: 'POST',
  headers: { 'project_id': env.BLOCKFROST_PROJECT_ID, 'Content-Type': 'application/json' },
  body: JSON.stringify({ query: qRest })
});
const dRest = await rRest.json();

localState = localState.applyCommitmentCollapsedUpdate(
  ledger.DustStateMerkleTreeCollapsedUpdate.deserialize(Buffer.from(dRest.data.c.update, 'hex'))
);
localState = localState.applyGenerationCollapsedUpdate(
  ledger.DustStateMerkleTreeCollapsedUpdate.deserialize(Buffer.from(dRest.data.g.update, 'hex'))
);
...
console.log('Target Block 884520 Comm Root: 73f0d85dbbeb72448b0f539b01d02d7cc921a1021ddc7967bfaba5dfe82b220206');
console.log('Target Block 884520 Gen Root:  733059fc03ad891577afd209677f8f25347317d52e5d94b5e97fad8533e24b9e04');
console.log('UTXO 1 mtIndex:', localState.utxos[0].mtIndex);
console.log('Wallet balance now:', localState.walletBalance(new Date()));
