...
console.log('1. Finding current on-chain Merkle tree tips...');
const tipGen = await findMaxEnd('dustGenerationMerkleTreeUpdate', 12284, 12284, 32767);
const tipComm = await findMaxEnd('dustCommitmentMerkleTreeUpdate', 91307, 91307, 131071);
console.log(`Current Generation Tip: ${tipGen}`);
console.log(`Current Commitment Tip: ${tipComm}`);

console.log('2. Fetching tree updates...');
const q = `query {
  ...
  genAfter: dustGenerationMerkleTreeUpdate(startIndex: 12284, endIndex: ${tipGen}) { update }
  commAfter: dustCommitmentMerkleTreeUpdate(startIndex: 91307, endIndex: ${tipComm}) { update }
}`;
const r = await fetch(indexerUrl, {
  method: 'POST',
  headers: { 'project_id': blockfrostKey, 'Content-Type': 'application/json' },
