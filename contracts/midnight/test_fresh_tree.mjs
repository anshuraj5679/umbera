...
localState = localState.applyGenerationCollapsedUpdate(
  ledger.DustStateMerkleTreeCollapsedUpdate.deserialize(Buffer.from(d.data.genUpdate.update, 'hex'))
);
localState = localState.applyCommitmentCollapsedUpdate(
  ledger.DustStateMerkleTreeCollapsedUpdate.deserialize(Buffer.from(d.data.commUpdate.update, 'hex'))
);
...
console.log('UTXOs count with fresh tree:', localState.utxos.length);
if (localState.utxos.length > 0) {
  const utxo = localState.utxos[0];
