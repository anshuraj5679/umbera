...
console.log('Binding transaction with provenTx.bind()...');
const boundTx = provenTx.bind();
console.log('Bound tx header:', Buffer.from(boundTx.serialize()).slice(0, 70).toString('utf8'));
console.log('Bound tx serialized length:', boundTx.serialize().length);
...
async function main() {
  ...
  console.log('Submitting bound transaction to Midnight Preview...');
  try {
    const result = await wallet.submitTransaction(boundTx);
