...
for (const startGen2 of [13107, 13108]) {
  for (const startComm2 of [115345, 115346]) {
    const q = `query {
      ...
      gen2: dustGenerationMerkleTreeUpdate(startIndex: ${startGen2}, endIndex: ${b.dustGenerationEndIndex - 1}) { update }
      comm2: dustCommitmentMerkleTreeUpdate(startIndex: ${startComm2}, endIndex: ${b.dustCommitmentEndIndex}) { update }
    }`;
    ...
    console.log('GraphQL errors:', d.errors);
    console.log('gen2 update exists:', !!d.data?.gen2?.update, 'comm2 update exists:', !!d.data?.comm2?.update);
    ...
    console.log(`\nConfig (gen2: ${startGen2}, comm2: ${startComm2}):`);
