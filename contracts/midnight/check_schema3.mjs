...
console.log('Query fields:', (d?.data?.__schema?.queryType?.fields || []).map(f => f.name).join(', '));
console.log('\nSubscription fields:', (d?.data?.__schema?.subscriptionType?.fields || []).map(f => `${f.name}(${f.args.map(a=>a.name).join(',')})`).join('\n  '));